...
class EmployeeControllerServiceTest {
    ...
    void getCountMaleFemale() {
        mockMvc.perform(get(String.format("http://localhost:%d/api/v1/employees/getCountMaleFemale", serverPort)))
...
/*
     ...
     ******************************************************************************************//*
        mockMvc.perform(
                ...
                .andExpect(MockMvcResultMatchers.content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.data").isMap())
                .andExpect(jsonPath("$.data.Female").value(21300.090909090908))
                .andExpect(jsonPath("$.data.Male").value(20850.0))
